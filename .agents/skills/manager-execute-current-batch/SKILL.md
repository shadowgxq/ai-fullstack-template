...
必读：[角色与初始化](references/role-contract.md)、[handoff](references/subagent-batch-handoff.md)、[输入契约](../manager-plan-from-doc/references/input-contract.md)、[证据](references/evidence-contract.md)、[自动修复](references/verify-auto-repair.md)。
...
3. 按 roles.routes.change 派发 architect 类只读角色返回草稿；Manager 单写目标活动目录。一个 Change 只有一个制品写入方。凡需要写入 Worker，同时生成最小 execution.yaml（可仅一项任务），与 tasks.md ID、允许路径及验收保持一致；在制品 gate/批准前完成，不在 apply 中临时改变已批准 contract。
...
2. 原生 Worker 不论串行/并行都使用已批准的 execution.yaml；简单任务可以只有一个节点。无任务图只适用于获明确许可的 Manager 直接串行实现，不派发未 claim 的 Writer；独立 Review 和全部 gate 不省略。
3. `task-ready --change <id>` 给出可并行任务与被阻塞原因。不要再用 selection.parallel 限制同一 change 的内部 DAG。
4. 通过真实 Codex 原生工具创建对应角色的等待分配线程；取得真实 agent ID。先执行 `task-claim`，再发完整任务让它开始写入。无法等待/分两步派发时停止原生派发；仅在已有明确许可下改为 Manager 串行实现并记录降级，不能先改代码再补 claim。
...
8. QA/UI 角色负责真实检查和证据采集；最终 Review 按 roles.routes.verify 派发非实现线程（模板默认 Sol reviewer）。`snapshot --change <id>` 的输出原样交给独立 Reviewer；返回 evidence-contract 指定报告。不要把实现者的长解释当作验收证据。
...
运行前使用 resolve-role --role <实际角色> 核对本地配置；这不是原生线程已成功加载、账号模型可用或权限已生效的证明。模型不可用不静默替换。
...
当前 wave 的 change 因上游尚未 apply、必需产物未生成而等待时，可先执行同 wave 内已批准且满足依赖的 apply；不跳过人工审批或转到后续 wave。真实 blocked 项仍按 Runner 停止规则处理。
原生任务失败会 block，不能直接重新领取绕过修复预算。显式 code reopen 保留代码、原报告及预算，但旧 claim 标为 superseded、任务勾选重置；必须重新验证，不能拿旧完成状态证明修复正确。
