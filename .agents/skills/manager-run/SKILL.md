...
1. round-begin --session <id> 核对冻结范围、轮数、未收尾轮次和先前检查点。只把返回 selection 交给 manager-execute-current-batch，不另选批次。
...
STOP 后的 session 不自动复活。用户处理验收/批准/阻塞后，以新的明确运行授权建立下一次 session；普通重启不等于批准。计划批次、停点或范围变化会使旧 scope_hash 不匹配，不能改 JSON 绕过。
