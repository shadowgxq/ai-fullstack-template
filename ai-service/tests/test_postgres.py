...
def environment():
    ...
    config = Settings(_env_file=None, database_url=dsn, api_key=TOKEN, scope=f"test-{uuid4()}")
    migrate(config)
    client = TestClient(create_app(config), headers={"Authorization": f"Bearer {TOKEN}"})
    return config, client, Store(dsn)

def create(client, key="one", text="hello"):
    return client.post("/api/v1/runs", json={"workflow": "echo.v1", "input": {"text": text}}, headers={"Idempotency-Key": key})
...
def test_idempotency_conflict_and_scope_isolation(environment):
    ...
    other = TestClient(create_app(config.model_copy(update={"scope": f"other-{uuid4()}"})), headers=client.headers)
...
def test_api_queue_worker_result_events_and_immutable_publish(environment):
    ...
    assert [e["event_type"] for e in client.get(base + "/events").json()] == ["queued", "running", "completed"]
