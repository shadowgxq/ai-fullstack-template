...
def settings():
    return Settings(
        _env_file=None,
        database_url="postgresql://test:test@localhost:1/ai_runtime_test",
        api_key=TOKEN,
    )


def test_hash_canonical_and_versioned():
    assert request_hash("v1", {"a": "1", "b": "2"}) == request_hash(
        "v1", {"b": "2", "a": "1"}
    )
...
@pytest.mark.parametrize(
    "kwargs",
    [{"database_url": "sqlite://"}, {"api_key": "short"}, {"poll_interval": 0}],
)
...
def test_graph_checkpoint_reuse_and_mid_graph_resume():
    ...
    graph.update_state(
        {"configurable": {"thread_id": "two"}}, {"text": "resume"}, as_node="__start__"
    )
