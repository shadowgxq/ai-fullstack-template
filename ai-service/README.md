...
通用 Agent Runtime 的可运行初始化，不是完整研究产品。使用 Python/FastAPI + LangGraph + PostgreSQL 单 API/单 Worker，不新增 Redis/Celery 前置依赖。工程规范入口见 [AI 服务工程规范](../docs/engineering/ai-service/README.md)。
...
`WorkflowRunner` 只负责框架无关分派，不是额外业务调度器，也不实现尚未进入范围的 Operation Ledger。现有 HTTP 契约、状态枚举和数据库结构不变。
...
安装包也提供显式入口；启动、迁移由使用者选择，不能以自检为由隐式执行：
...
旧 Makefile、Uvicorn 工厂和 `python -m ai_service.worker` 入口继续可用。API 在 8001；`/health` 是存活探针，`/ready` 检查应用表和 Checkpoint 表。迁移显式执行，不在 API import、启动或请求中自动建表。
...
使用返回 run_id 查询 `/api/v1/runs/{run_id}`、`/result`、`/events`，均需相同认证。环境变量仍为 `AI_DATABASE_URL`、`AI_API_KEY`、`AI_SCOPE`、`AI_POLL_INTERVAL`，格式和默认值见 [config.py](src/ai_service/config.py)；服务凭证不能放进 VITE 变量。
...
未配置测试数据库时 integration 明确 skipped，不算 PostgreSQL 验证通过。CI 使用真实 PostgreSQL，保留失锁、重复创建和 Checkpoint 恢复用例，新增独立 CLI Worker、未知版本拒绝和离线自检测试。构建后将 wheel 安装到测试环境，在源码目录外运行 module/console 自检，验证安装入口与迁移资源；不是仅验证 editable checkout。
...
当前仍只有确定性 `echo.v1`。基础初始化不等于完整 Runtime：真实模型、ModelPort/ToolRegistry、Operation Ledger/预算/unknown、人工回答、取消、修订、SSE、Artifact 审核发布与生产多租户仍未实现，见 [当前与目标](../docs/architecture/README.md)。不得把 echo Node 直接替换为付费调用。
