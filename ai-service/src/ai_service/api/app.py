...
from ai_service.agent_core.contracts import (
    IdempotencyConflict,
    ResultPending,
    RunNotFound,
)
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    @app.get(
        "/ready",
        operation_id="aiReady",
        responses={503: {"description": "Storage not initialized or unavailable"}},
    )
    ...
    @app.post(
        ...
        responses={**errors, 409: {"description": "Idempotency conflict"}},
    )
    def create_run(
        body: CreateRun,
        idempotency_key: Annotated[
            str, Header(min_length=1, max_length=128, pattern=r"^[!-~]+$")
        ],
    ):
    ...
    @app.get(
        ...
        dependencies=dependency,
        operation_id="getRun",
        responses={**errors, 404: {"description": "Run not found"}},
    )
    ...
    @app.get(
        ...
        response_model=EchoInput,
        dependencies=dependency,
        operation_id="getRunResult",
        responses={
            **errors,
            404: {"description": "Run not found"},
            409: {"description": "Result unavailable"},
        },
    )
