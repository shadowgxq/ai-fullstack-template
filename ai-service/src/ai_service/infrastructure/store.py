...
PUBLIC_FIELDS = (
    "run_id", "workflow", "status", "input", "output", "error_code",
...
class Store:
    ...
    def ready(self) -> None:
        with self.connect() as connection:
            for table in ("ai_schema_migrations", "ai_runs", "ai_commands", "ai_events", "checkpoints"):
                connection.execute(psycopg.sql.SQL("SELECT 1 FROM {} LIMIT 1").format(
                    psycopg.sql.Identifier(table)
                ))

    def create(self, scope: str, key: str, digest: str, workflow: str, payload: dict) -> dict:
        with self.connect() as connection, connection.transaction():
            ...
            """, (uuid4(), scope, key, digest, workflow, Jsonb(payload))).fetchone()
            if row is None:
                existing = connection.execute(
                    "SELECT * FROM ai_runs WHERE scope = %s AND request_key = %s", (scope, key)
                ).fetchone()
                if existing["request_hash"] != digest:
                    raise IdempotencyConflict()
                return public_view(existing)
            connection.execute("INSERT INTO ai_commands (run_id) VALUES (%s)", (row["run_id"],))
    ...
    def get(self, scope: str, run_id: str) -> dict:
        with self.connect() as connection:
            row = connection.execute(
                "SELECT * FROM ai_runs WHERE scope = %s AND run_id = %s", (scope, run_id)
            ).fetchone()
        if row is None:
            raise RunNotFound()
        return public_view(row)
    ...
    def finish(self, connection, run_id: str, output: dict | None, error_code: str | None = None) -> None:
        ...
        with connection.transaction():
            ...
            """, (status, Jsonb(output) if output is not None else None, error_code, run_id)).fetchone()
            if row is None:
                return
