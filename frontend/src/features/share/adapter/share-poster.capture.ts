...
/**
 * Captures the already-rendered poster node with html-to-image. The library serializes computed
 * styles, pseudo-elements, fonts, and images into an SVG before rasterizing it, so responsive
 * container styles are kept from the same node the user sees in the share dialog.
 */
export async function captureSharePoster(
  ...
  try {
    ...
    // Measure after fonts, images, and the dialog layout settle. Measuring before that point can
    // lock an opening H5 dialog's temporary width/height into the exported SVG.
    ...
    if (!blob) {
      throw new ShareError('failed', 'Poster PNG could not be created.');
    }
    return blob;
  } catch (error) {
