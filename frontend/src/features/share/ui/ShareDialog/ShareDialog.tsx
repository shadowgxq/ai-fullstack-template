...
export type ShareDialogProps = {
  ...
  /**
   * The business feature owns the preview/card markup. The share layer captures
   * this independent node and provides the modal shell and platform actions.
   */
  poster: ReactNode;
  /** Optional status for a caller that supplies its own `posterFile`. */
  posterStatus?: SharePosterStatus;
  onOpenChange: (open: boolean) => void;
  surface?: ShareSurface;
  /** Keep the platform menu aligned with ai-berkshire by default. */
...
type PosterCaptureState = Readonly<{
  scope: string;
  status: SharePosterStatus;
  file?: File;
}>;
  ...
  const actionClassName =
    variant === 'primary'
      ...
      : variant === 'secondary'
        ...
        : variant === 'platform'
          ? 'h-auto min-h-[var(--popover-tile-height)] min-w-0 w-full flex-col px-[var(--popover-tile-padding-x)] text-[var(--font-size-xs)]'
...
export function ShareDialog({
  ...
  useEffect(() => {
    ...
    const timer = globalThis.setTimeout(() => {
      ...
      void captureSharePoster(target, {
        pixelRatio: 2,
  ...
  const shareActions = share.actions.filter((descriptor) => {
    if (!isShareChannelDescriptor(descriptor)) return false;
    return includeNativeShare || descriptor.action !== 'native';
  });
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <DialogFooter className="mt-0 shrink-0 flex-col border-t border-border-subtle bg-popover px-[var(--dialog-padding)] pt-[var(--space-4)] pb-[var(--dialog-padding)] sm:flex-row sm:items-center sm:justify-between">
          {imageIsPreparing ? (
            <p
              className="text-muted-foreground mb-[var(--space-3)] flex items-center gap-[var(--button-gap)] text-[var(--font-size-sm)] leading-[var(--line-height-sm)]"
              role="status"
            >
              <LoaderCircle
            ...
            <p
              className="text-destructive mb-[var(--space-3)] text-[var(--font-size-sm)] leading-[var(--line-height-sm)]"
              role="alert"
            >
          ...
          <div className="flex w-full flex-col gap-[var(--space-3)] sm:flex-row sm:items-center sm:justify-between">
            {feedback ? (
              <p
                className={cn(
                  'shrink-0 text-[var(--font-size-sm)] leading-[var(--line-height-sm)]',
            ...
            >
              ...
              {shareActions.length === 0 && copyActions.length === 0 && !downloadAction ? (
                <p className="text-muted-foreground text-[var(--font-size-sm)] leading-[var(--line-height-sm)]">
                  {imageIsPreparing
                    ? (preparingImageLabel ?? t('share.preparingImage'))
