...
`pnpm check` 只包含本端 typecheck、Lint、测试与代码格式检查，不要求 Python 或其他服务的工具环境。仓库级文档、Manager/repair 校验统一在根目录执行 `make docs`；跨端检查使用根 `make check`。根 Compose 中使用 Nginx 提供构建产物和 `/api` 代理，访问 8080。启动命令见 [根 README](../README.md)，实现细则见 [专项导航](../docs/engineering/frontend/README.md)。
