...
| 状态 | 能力 | 实现约束 |
|---|---|---|
| 已有基础初始化 | 包入口、静态注册与注入、服务认证、受信 scope、创建幂等、JSON 事件轮询、单 Worker、Checkpoint 恢复、Worker JSON 日志 | 从现有代码和测试扩展；离线自检不等于数据库就绪，测试存在不等于本次已运行 |
...
基础初始化代码已落地，不是只有规范或空目录；但 WorkflowRunner 只是注册分派，不冒充完整的长期执行/副作用引擎。安装与验证入口统一见 [AI README](../../../../ai-service/README.md)。
