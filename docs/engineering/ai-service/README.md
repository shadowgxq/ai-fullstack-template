...
`api → application → agent_core`；workflows 组合业务步骤，infrastructure 提供数据库等适配。core 不依赖模型 SDK 或业务模块。初始化 application 使用 Store 注入，后续第二个实现出现时再抽端口协议。

API 只收命令与读快照；Worker 执行图。Run 与待执行命令同事务写入，事件先持久化再读取；LangGraph checkpoint 不替代应用状态。持有 PostgreSQL 会话级排他锁的同一连接完成应用状态写入；当前只能一个 Worker。
