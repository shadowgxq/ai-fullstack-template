...
| 类型 | 约定 | 示例 |
|---|---|---|
| 模块 / 包 | 语言写法见 [Python 公共规则](../../common/python.md)，本端按层命名 | `user_repository.py`、`api/v1/` |
...
- 按资源域在需要的层新增模块，沿用 auth/user 的现有模式；仅有实际职责时才创建，不要求每个接口生成 model、repository、service 全套空文件。业务逻辑按层归类，不打包成巨型文件。
...
- 共享抽象判断见 [公共代码质量](../../common/code-quality.md#复用与抽象)；领域纯函数留在领域，只有无业务依赖的横切能力才进 `core`。
