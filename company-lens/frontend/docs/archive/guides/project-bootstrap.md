...
模板 `package.json` 默认预装 `react-router-dom`、`axios`、`@tanstack/react-query`、`zustand`、Radix UI 基础包、`lucide-react`、`clsx`、`i18next`、`react-i18next`、`vitest` 和 React Testing Library，并默认接入 theme 与 i18n。它们是公司默认初始化选型，不是使用方项目不可替换的强制技术约束。
