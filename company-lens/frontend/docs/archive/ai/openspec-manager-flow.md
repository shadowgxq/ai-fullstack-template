...
把「需求 → OpenSpec → 批次 → 执行 → 回写 → 归档」用最少状态跑通，并按互联网研发流程叠加多角色协作：**产品提需求 →（可选）需求评审会 → 生成方案 →（可选）方案评审 → 开发 → 测试 → bugfix**。Manager 起管理/编排作用，具体工作派给角色化 subagent。**两处多角色评审都是可选项、默认关闭**：需求评审会靠显式调用开启，方案评审门禁靠运行参数 `review=on` 开启；默认主链路只保留 validate + apply 后 verify 两道硬门禁和 change→apply 人工边界。
...
| `manager-plan-from-doc`         | 排期                           | 手动                   | 把 PRD 规划成 `plan.yaml` 的 requirements/openspec/batches/waves                                                                            |
| `manager-execute-current-batch` | 生成 + 开发 + 验收（评审可选） | 手动 / Goal 循环       | 跑 change/apply 批次；方案评审门禁由 `review=on` 参数开启（默认 off 直接放行），apply 后 verify 门禁必跑，从不归档                          |
...
| `manager-bugfix`                | 修复回路                       | 手动                   | 按归档状态分流修复，Repair Loop 复验解锁，闭环产出 `findings/`                                                                              |
...
- **state**（当前 phase 能否执行/到哪）：`planned` / `ready` / `in-progress` / `blocked` / `cancelled`
...
docs/prd/features/NN-*.md（产品提需求）
  ...
  │
  ② manager-plan-from-doc ── 规划 requirements/openspec/batches（preview 显示需求评审状态，未评审不阻断）
  │
  ...
  │     按 tasks.md 分节派发（后端先行 → 前端）实现代码
  │     → apply 后 verify AI 门禁（必跑；roles.yaml verify 路由：qa + 含前端则 ui-ux-reviewer）
  │         无 CRITICAL → advance 到 archive
  │         有 CRITICAL → block
  │
  ④ manager-bugfix（block 或人工发现 bug 时）
  │     分流 → Repair Loop 回派修复 → 复验 → start+advance 解锁 → 产出 findings/
  │
...
| **验收**（verify，apply 后）            | **开（必跑）** | 不可关                                    | AI 门禁                       | 无 CRITICAL → advance                   | block → manager-bugfix               |
...
AI 闸只写 `approved`、从不写 `blocked`（那是 verify 的活）、从不自行跨进 apply。人工不做逐条 set-review，只处理升级待决 + change→apply 边界确认。**删 roles.yaml 里 `gate: auto` 一行即回退成人工评审**（意见仅供参考，只有用户 `set-review approved` 放行）。
...
### bugfix Repair Loop + findings

verify block 或人工发现 bug 时走 `manager-bugfix`，分流三句话：

1. **绑定未归档 change**（含 blocked entry）→ Repair Loop 修原 change；
...
| `openspec/changes/<id>/verify-report.md` | verify/复验裁决（追加日期小节）                         | 复验聚焦、bugfix 输入     |
...
| `block --change <id>... --reason <text>`                          | 阻塞记录                                                                                               |
...
| verify 门禁 | `opsx:verify`  | `openspec-verify-change`  | execute           | 核对实现与制品一致性，CRITICAL 阻塞 |
...
| `MANAGER-RUN: STOP(blocked)`     | 出现 blocked 条目 → 指向 `manager-bugfix`                                                   |
...
- **作用**：PRD → `plan.yaml` 的 requirements/openspec/batches/waves。只规划，不建 artifact、不实现、不执行、不归档。本地 PRD 回写「Manager 规划覆盖」marker。
- **粒度默认**：一 requirement → 一 change，data-flow 在该 change 内设计；仅复用/并行/独立风险/体量过大时才把契约拆独立 change。
- **preview**：显示每个本地 PRD 来源的 `manager-prd-review` 状态（已通过 YYYY-MM-DD / 未评审）——prd-review 是排期前**可选**的 intent 闸（非机器门禁，默认跳过、未评审不阻断，只显式提示由用户决定）。
- 只写初始态（不写 `behavior`/`review`），转移规则指向 execute 不复述。
...
- **门禁**：change 成功 = 验收 + checks + `openspec validate`，完成写 `review: pending` → 按运行参数 `review` 处理（默认 off 当轮直接放行；on 时走方案评审门禁）；apply 成功 = 验收 + checks + `openspec validate --strict` + verify subagent 无 CRITICAL（必跑）。失败 → block。
- **subagent handoff**：每个派发生成 handoff（含 `agent_type`、`tasks_section`、入口、required skill、允许输入、推导 checks、并行标志），subagent 不改 plan.yaml、不跑 archive。
- **并行**：仅 `next` 说 `parallel: true` 且输入独立、不撞 `shared_write_points` 时并行；否则串行。

### manager-bugfix（修复回路）

见上「bugfix Repair Loop + findings」。**手动触发**（显式 `$manager-bugfix`、要求走修复流程、或修复 blocked entry 时），不因提及 bug 自动触发。references 三件套：`root-cause-checklist.md` / `backwrite-matrix.md` / `finding-template.md`。
