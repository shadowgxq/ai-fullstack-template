...
- 先按内容语义定义文本预算，再选择单行省略、固定行数截断或自然换行；不要对所有文本统一省略。
- 单行省略必须同时具备受约束宽度、`min-width: 0`、`overflow: hidden`、`text-overflow: ellipsis` 和 `white-space: nowrap`。位于 flex/grid 中时，父级 track 使用 `minmax(0, 1fr)` 或为 flex item 设置 `min-width: 0`。
- 多行截断必须同时具备 `display: -webkit-box`、`overflow: hidden`、`overflow-wrap: anywhere`、`-webkit-box-orient: vertical` 和明确的 `-webkit-line-clamp`。
- 文本旁有图标、状态点或固定操作时，只截断独立的文本容器，固定元素设置 `flex: 0 0 auto`，避免图标被压缩或跟随省略。
- 固定画布、海报、图表标签等格式必须同时限制字段行数和集合数量；CSS 负责最终视觉边界，model/mapper 负责生成适合展示的摘要，不修改其他用途所需的完整原文。
- 视觉截断不能从 DOM 或可访问名称中删除完整内容。关键任务信息如果被截断，必须提供详情入口、tooltip、可访问描述或其他完整内容来源。
- 不为纯 CSS 截断默认新增 React 组件。只有 tooltip、展开、可访问描述等行为在两个以上稳定场景一致时，才考虑抽取 shared UI。
