...
- 初始值：没有有效持久化值时默认使用 light，之后以用户显式选择并持久化的 light/dark 为准。
...
**开发态提示禁止写进 `locales/*.json`。**

- 开发态提示走 `console`，或在渲染层按数据源条件显示（如 `dataSource === 'mock'` 时才渲染），切到真实 API 自动消失。
- 确属产品决策要展示的开发态文案（如 upload 接口未就绪时的显式 mock 标识），登记到 `scripts/lint-locales.allowlist.json` 并写明保留理由和摘除条件。
- `pnpm lint:locales`（已在 `pnpm check` 链上）机械拦截：命中 `开发态`/`admin123`/`dev`/`TODO`/`mock` 且未登记即失败；白名单条目失效同样失败，强制与文案同步清理。项目 opt-out i18n 时该检查自动跳过。

## opt-out（项目基线变更）

不要把 opt-out 当成删除单个目录的局部修改。执行前先搜索全部 import、provider、测试和演示调用；修改后按影响范围验证，范围见根工程规范。
...
- **同步项目规则**：更新相关导航、`docs/frontend/standards/frontend-development.md`、本指南和依赖清单，避免 Agent 继续按已移除的基线实现。
