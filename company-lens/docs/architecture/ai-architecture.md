...
> 当前实现与验证见[项目状态](../project-status.md)；规划交接见[架构实施规划输入](../engineering/architecture-planning.md)。
...
| 类别 | 当前基线 | 状态与依据 |
|---|---|---|
| 已确认工程边界 | `frontend/` + `ai-service/`；保留已有 CSS Modules / Radix UI、视觉与交互，按新合同替换 API/DTO | 前端已迁入，接口适配未实施；见[前端迁入记录](../references/frontend-import.md) |
| 领域与运行依据 | 执行、业务两份 v1.1，系统接入详细设计 v1.0 | 设计合同；来源与合并说明见[导入记录](../references/architecture-import.md) |
| 系统设计建议 | 单机共享产物卷、单 Worker、事务边界、发布竞态、快照游标、迁移和质量目标 | 待采纳、实施及验证；见第10节 |
| 实现基线 | 已有前端代码与锁文件，AI 服务尚未初始化 | 进度只在[项目状态](../project-status.md)维护 |

### 0.3 文档职责与变更规则

[文档导航](../README.md)定义各文档职责。本文维护系统级行为与约束，两份 v1.1 维护执行内核和研究领域规则，[系统接入详细设计](../design/system-integration-details.md)细化事务、DTO、存储、迁移及验证接缝。系统新增字段和投影以接入详细设计为准；金融公式、来源政策和报告准出仍由业务设计维护。发现实质冲突时先记录决策，再同步相关合同。

[PRD](../product/prd.md)与[原始来源](../product/sources/ai-berkshire%20公司研究模块.md)保留产品意图；当前任务在 `docs/changes/`，Manager/OpenSpec 仅为可选规划入口。`AGENTS.md` 只做导航。
...
保留 S01–S11、T01–T06、M01–M10。其完整范围仍在业务详细设计；第1节明确哪些属于当前 P0，哪些只是演进背景。
...
以下数值是设计建议的**启动约束/验收目标**，不是生产性能事实。纳入配置后可调整，但调整需记录与回归，不允许模型自行扩大。
...
| 单次/总请求、token、总期限 | live前必填；实际额度待配置 | 缺配置不得发付费请求 |
...
完整状态见[架构决策索引](decisions/README.md)。ADR-0001 保留单仓库决定，其前端和进程细节分别由 ADR-0002、0003 承接；其余记录新增系统级取舍。

| ADR | 决定 | 状态与主要代价 |
|---|---|---|
| [ADR-0002](decisions/0002-preserve-frontend.md) | 前端保留CSS Modules/Radix与交互，只改接入 | 用户已明确；旧DTO适配需要单独回归 |
...
|---|---|---|---|
| O-01 | 已迁入前端的认证、路由、DTO 与事件适配点 | 实施负责人；对应接入变更前 | 盘点相关消费者；现有代码存在不表示新合同已接通 |
...
| O-08 | 若采用 Manager：既有任务承接、计划依赖及文件所有权 | 规划者；实际计划预览前 | 当前仍用 docs/changes；未证明隔离时不并行实施 |

以上不是要求继续增加平台，而是应阻断对应风险路径的具体条件。数据不可得、供应商不透明、长财报解析错误、公开建议/来源使用限制、单机故障与旧UI合同差异是首期主要风险。

### 10.3 合同维护边界

前端沿用现有工程；API、Worker 与持久化按本文及接入设计实施。checkpoint 只管理图恢复，正式交付由应用产物及发布门禁决定。旧版设计和来源快照位于[历史归档](../archive/README.md)，不作为当前实现指令。开发流程及状态维护见[工程规范](../engineering/ai-development-standard.md)。
...
此表是分解建议，不是已生成的Manager计划。V3/V4若高度共享路由、状态文件或本来是一条完整路径，可合并交付；若V1没有独立可测服务结果，则并入V2。不要因为表中有5行就固定生成5个change。

与旧R0–R5的关系：V1是D0/R1/R2宿主机制的提前验证，不改变R0数据风险需尽早验证；V2完成R1并满足R2三市场数据基础，V3–V5闭合R2 Web首期。之后R3完整研究、R4团队、R5专项仍从业务PRD另行规划，R5不必等待R4。

候选切片尚未进行文件所有权分析，后续实施默认串行。API schema、run projection、前端路由、全局token、迁移编号、注册表和锁文件均可能是共享写点。
...
本文是系统需求主来源。明确交接三份支持输入：执行设计 `core-runtime-design`、业务设计 `research-domain-design`、系统接入设计 `system-integration-detail`；它们为相关实现与验证提供必需合同。完整路径、消费者和章节范围统一见[架构实施规划输入](../engineering/architecture-planning.md)。ADR 与外部参考资料用于溯源，不自动扩展实施范围。

## 附录 C. 一致性核对

系统行为与质量验证统一见第9节，故障窗口见第5.7节，实施验证接缝见[系统接入设计第7节](../design/system-integration-details.md)。文档完整性不代表实现或运行验收通过。

## 附录 D. 来源与验证范围（非执行输入声明）

以下为原始文档包使用的参考资料，随来源一并保留。业务来源由业务设计维护；原包验证声明只适用于原包，当前导入范围见[导入记录](../references/architecture-import.md)。
...
原包记录的查阅日期为 2026-09-16。实际 API 兼容性以实现时锁文件与相关契约验证为准；这些引用不构成当前实现或运行验证证据。
