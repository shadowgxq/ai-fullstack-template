...
第一次阅读先看第1节（首期边界）、系统技术架构与第11节（工程落点）、第15节（Web接入）；实现首条闭环时重点看第5–8节。第9.7–9.9节和动态规划部分是按需扩展，不是首期待办清单。公司的来源、数字、估值和上游迁移细节请读配套业务文档。

## 1. 实现目标、工程边界与首期剖面

### 1.1 已确认边界

工程只有 **`frontend` + `ai-service`** 两个应用工程；通用能力与公司研究业务是 `ai-service` 内部的模块分层。本文定义执行层合同，实际实现见[项目状态](../project-status.md)。
...
冲突时：用户明确边界优先；运行状态和 API 以本文为准；研究数据、模式与准出以业务文档为准。两个文档不重复定义不同版本的同一状态机。
...
学习参考为 `shadowgxq/ai-agent-roadmap`；原设计核对了下表入口，未固定完整提交快照。实际复用须登记 commit/hash 并验证边界。[R01–R06、R13]
...
v1.0 提到的 `agent-mini` context/cache/MCP/RAG/evals 等入口保留为候选学习资料，尚未逐项审计，不能作为已验证实现。后续启用相应扩展时单独核对。

**抽取步骤：** 先编写新模块的边界测试 → 只复制或重写被当前测试需要的函数 → 去除工单/代码工作区等领域字段 → 接入统一调用和持久化 → 用公司研究场景验收。不要把三个旧教程项目设为生产依赖，也不让新服务 import 旧仓库相对路径。

## 3. 技术选型与依赖方向

系统组成、技术栈与模块依赖统一见[系统技术架构第2–3节](../architecture/ai-architecture.md)。本设计负责运行合同；具体库版本在 Python 初始化时锁定并验证。
...
模块位置见[系统技术架构](../architecture/ai-architecture.md)，开发流程见[工程规范](../engineering/ai-development-standard.md)。同一包提供 API 与 Worker，只有一个依赖锁和应用迁移体系；core 不包含公司、证券、DCF 或评分字段。

R0–R5 里程碑统一由[业务设计第16节](AI公司研究_业务架构与接入设计_v1.1.md)维护。公共能力沿研究切片实现，Planner/MCP/向量检索/长期记忆不是首期必需项。
...
下列来源用于设计溯源，核对范围以第2节为准；库接口需在实际锁定版本下验证。运行状态见[项目状态](../project-status.md)。
...
实现时将所采用教程文件固定为具体 commit+hash；`master` 链接是浮动参考入口。外部库版本进入实际 lock 文件后，再完成契约/兼容性测试。
