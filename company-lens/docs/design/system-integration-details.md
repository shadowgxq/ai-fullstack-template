...
下文新增列名、DTO投影、控制事务、事件算法和配置均为系统级工程细化。字段可在实现阶段按实际框架调整，但不能改变可观察行为；修改公共字段时同步OpenAPI/事件schema及客户端。不要因为本文给出7类逻辑表，就为每个业务对象生成一张表。
...
每个实施变更记录实际命令、退出码、测试环境/版本、已运行与未运行场景及数据证据位置。这里只定义验收接缝；当前记录位置遵循[工程规范](../engineering/ai-development-standard.md)。
